...
@register_env("DiceCleanup-v1", max_episode_steps=400)
class DiceCleanupEnv(BaseEnv):
    """
    **Task Description:**
    The goal is to open the drawer, pick up the dice and put it inside the drawer, and close the drawer.

    **Randomizations:**
    - dice initial position and orientation
    ...
    **Success Conditions:**
    ...
    def evaluate(self):
        ...
        for c in all_contacts:
            b0, b1 = c.bodies
            if 'drawer' in b0.name and 'dice' in b1.name:
                is_obj_in_drawer = torch.ones_like(is_obj_grasped, dtype=torch.bool)
            if 'drawer' in b1.name and 'dice' in b0.name:
                is_obj_in_drawer = torch.ones_like(is_obj_grasped, dtype=torch.bool)
    ...
    def get_prompt_content(self):
        ...
        obb = trimesh.primitives.Box(extents=extents, transform=T_final)
        dice_extent = extents.tolist()
        dice_transform = T_final.copy()
        ...
        closing, center = grasp_info["closing"], grasp_info["center"]
        grasp_pose = self.agent.build_grasp_pose(approaching, closing, center)
        grasp_dice_pose = sapien.Pose([0, 0, -0.01]) * grasp_pose

        place_dice_pos = pin_pose.p - pull_dir * 0.20


        instruction_for_stage_id = [
            'You need to open the drawer first.',
            'The drawer has already been opened. Now you can pick up the dice and put it inside the drawer.',
            'The dice has been put inside the drawer. Now you only need to close the drawer.',
            'The task is completed successfully.',
        ]

        info_for_stage_id = [
            'When you are completing the open drawer stage, the place_dice_when_drawer_open_pos is wrong and useless. You should **never** close the gripper because you can only use one side of the gripper to pin on the handle. Always remember the pull_direction is definitely correct, and maintain the gripper open when pull the drawer. Never use the grasp_dice_pos and grasp_dice_quat in this stage because they are wrong.',
            'When you are completing the pick and place dice stage, you can use the place_dice_when_drawer_open_pos to help you move to a proper position above the drawer to put the dice in. Remember, if the gripper is currently pined on the handle, you must move up first to a higher position (z>0.2) to avoid collisions with the drawer, then go to pick the dice.',
        ...
        return {
            'task_desc': 'Hook the handle with the gripper of a single side to open the drawer, pick up the dice and put it inside the drawer, and close the drawer.',
            'ground_truth': {
    ...
    def get_fix_prompt_content(self):
        ...
        return {
            ...
            'ground_truth': {
                'current_stage': instruction_for_stage_id[self.check_stage()],
                'current_tiny_task': 'Pick up the dice.',
                # 'last_error': 'The gripper position xy is not aligned with the dice position xy.',
                # 'current_stage_info': info_for_stage_id[self.check_stage()],
                'tcp_pos': self.agent.tcp.pose.p.tolist(),
                ...
                # 'grasp_dice_pos': grasp_dice_pose.p.tolist(),               
                # 'grasp_dice_quat': grasp_dice_pose.q.tolist(),
                'dice_extent': dice_extent,
                'dice_center_pos': dice_center_pos.tolist(),


                # 'dice_transform': dice_transform.tolist(),

                
                'additional_info': 'When you grasp the dice, you can not directly use the dice_center_pos because you can not grasp the dice from its center. You should use the extent to compute a better grasping position, such as the top rim position of the dice.',
                # 'pull_open_length': 0.21,
                # 'place_dice_when_drawer_open_pos': place_dice_pos.tolist(),
                # 'additional_info': 'Always check first, if you want to move to some higher position, and the ground truth shows the gripper is in some lower position (z<0.2), always move up first to a higher position. When you want to approach some determined position, always reach to a little above first, then move down to the target position. When you finish reach one target, and want to move to another target, always move up first to avoid collision. For this task, use 0.15 meters above the target position as the approach position. When you grasp the dice, use 0.21 meters above the grasp pose to avoid collisions with the drawer. place_dice_when_drawer_open_pos only works when you are in the stage of putting dice into the drawer, So do not consider it in other stages.'
            }
